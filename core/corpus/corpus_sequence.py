...
class CorpusSequence:
    ...
    def from_json(cls, path: str | Path):
        ...
        return cls(corpus=data["corpus"], inscriptions=inscriptions)
    ...
    def validate(self):
        ...
        for ins in self.inscriptions:
            ...
            if not all(isinstance(x, str) for x in ins.signs):
                errors.append(f"{ins.id}: sign identifiers must be strings")
            if not (0.0 <= ins.confidence <= 1.0):
                errors.append(f"{ins.id}: confidence out of range")
            for p in ins.damaged_positions:
                if p < 0 or p >= len(ins.signs):
                    errors.append(f"{ins.id}: damaged position {p} out of range")
        return errors
